...
// Dining locations where swipers can be available
export const swipeLocations = pgTable("swipe_locations", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
...
// Swiper goes "online" at a specific location
export const swipeSessions = pgTable(
  "swipe_sessions",
  {
    ...
    locationId: integer("location_id")
      .notNull()
      .references(() => swipeLocations.id),
    status: text("status", { enum: ["active", "offline"] }).notNull().default("active"),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow(),
  },
  (table) => ({
    swiperActiveIdx: index("idx_swipe_sessions_swiper_active").on(table.swiperId, table.status),
    locationActiveIdx: index("idx_swipe_sessions_location_active").on(table.locationId, table.status),
  })
);

// Buyer's swipe request — payment captured at creation time
export const swipeRequests = pgTable(
  "swipe_requests",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    ...
    locationId: integer("location_id")
      ...
      .references(() => swipeLocations.id),
    status: text("status", {
      enum: ["pending_payment", "pending", "matched", "completed", "cancelled", "expired"],
    })
      .notNull()
  ...
  (table) => ({
    buyerStatusIdx: index("idx_swipe_requests_buyer_status").on(table.buyerId, table.status),
    locationStatusIdx: index("idx_swipe_requests_location_status").on(table.locationId, table.status),
  })
);

// Links a request to a specific swiper
export const swipeMatches = pgTable(
  "swipe_matches",
  {
    ...
    requestId: uuid("request_id")
      .notNull()
      .references(() => swipeRequests.id, { onDelete: "cascade" }),
    ...
    swiperConfirmed: boolean("swiper_confirmed").default(false),
  ...
  (table) => ({
    requestIdx: index("idx_swipe_matches_request").on(table.requestId),
    swiperIdx: index("idx_swipe_matches_swiper").on(table.swiperId),
  })
);

// Earnings owed to swipers after a completed match
export const swipeEarnings = pgTable(
  "swipe_earnings",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    swiperId: uuid("swiper_id")
    ...
    matchId: uuid("match_id")
      .notNull()
      .references(() => swipeMatches.id),
    amountCents: integer("amount_cents").notNull().default(400),
    status: text("status", { enum: ["pending", "paid"] }).notNull().default("pending"),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
  },
  (table) => ({
    swiperStatusIdx: index("idx_swipe_earnings_swiper_status").on(table.swiperId, table.status),
  })
);

// Per-user swipe profile (venmo handle, swiper opt-in, earnings total)
export const swipeProfiles = pgTable("swipe_profiles", {
  userId: uuid("user_id")
    .primaryKey()
    .references(() => users.id, { onDelete: "cascade" }),
  venmoHandle: text("venmo_handle"),
  isSwiperEnabled: boolean("is_swiper_enabled").default(false),
