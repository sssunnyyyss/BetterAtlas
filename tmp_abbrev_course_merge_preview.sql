...
ranked AS (
  SELECT
    ...
    row_number() OVER (PARTITION BY s.source_id ORDER BY s.shared_course_count DESC, s.target_id ASC) AS rn_shared,
    row_number() OVER (
      PARTITION BY s.source_id
      ORDER BY (CASE WHEN s.dept_match THEN 0 ELSE 1 END), s.target_id ASC
    ) AS rn_dept
  FROM scored s
),
chosen AS (
